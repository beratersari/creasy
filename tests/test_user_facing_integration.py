...
from mireviewer.gitlab.events import Ignore, classify_webhook
from mireviewer.jobs.manager import Manager
from mireviewer.jobs.worker import OpenCodeRunner
from mireviewer.opencode.session import last_assistant_text, turn_assistant_text
...
def _assign_body(*, draft: bool = False):
    return {
...
def test_new_commits_and_mark_as_ready_do_not_start_a_review(tmp_config, tmp_path: Path):
    ...
    mark-as-ready must not spend a slot; assigning the bot still runs.
    """
    client, manager, state, httpd = _boot(tmp_config, tmp_path, assistants=[REVIEW_MARKDOWN])
    try:
        push = {
        ...
        headers = {"X-Gitlab-Token": "secret"}
        assert client.post("/mireviewer/webhook/gitlab", json=push, headers=headers).json()["status"] == "ignored"
        assert client.post("/mireviewer/webhook/gitlab", json=ready, headers=headers).json()["status"] == "ignored"
        assert manager.store.list_all() == []
        res = client.post("/mireviewer/webhook/gitlab", json=_assign_body(), headers=headers)
...
def test_draft_open_is_skipped_but_explicit_review_still_runs(tmp_config, tmp_path: Path):
    ...
    try:
        ...
        skipped = client.post("/mireviewer/webhook/gitlab", json=draft_open, headers={"X-Gitlab-Token": "secret"})
        assert skipped.json()["status"] == "ignored"
        note = _assign_body(draft=True)
...
def test_ask_through_the_http_api(tmp_config, tmp_path: Path):
    client, manager, state, httpd = _boot(
...
def test_queued_job_is_live_and_jobs_list_has_no_http_poll(tmp_config, tmp_path: Path):
    ...
    """
    from mireviewer.api import dashboard as dash
