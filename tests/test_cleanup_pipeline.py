"""OSM PLAN §8 kill/delete helpers, ported. Cascade trigger is MR close/merge."""
...
def test_may_kill_never_allows_manager_or_system() -> None:
...
def test_kill_pid_never_sends_signal_to_manager(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        killmod.os, "killpg", lambda *a, **k: spawned.append(("killpg", a)), raising=False
    )
    kill_pid(os.getpid())
    kill_pid(os.getppid())
    kill_pid(1)
...
def test_windows_cwd_candidate_is_clone_tools_only() -> None:
    ...
    assert not windows_cwd_candidate(
        "", r'"C:\creasy\.venv\Scripts\python.exe" -m creasy.app'
    )
...
def test_iter_windows_processes_does_not_snapshot(monkeypatch, tmp_path: Path) -> None:
    ...
    stop_job_holders(_job(job_id="job_no_snap"), clone)
...
def test_stop_job_holders_windows_skips_rm_until_delete(tmp_path: Path, monkeypatch) -> None:
    ...
    clone = tmp_path / "T-1"
    clone.mkdir()
    stop_job_holders(_job(job_id="job_rm1"), clone)
    assert called == ["locks"]
