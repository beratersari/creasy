...
from mireviewer.azure.auth import azure_basic_auth, azure_basic_user
from mireviewer.logging import get_logger, log_command, log_command_result, log_fail, log_ok, redact_userinfo
...
def askpass_path() -> Path:
    if os.name == "nt":
        ...
        path = root / "mireviewer" / "git-askpass.cmd"
        body = _ASKPASS_CMD
    else:
        root = Path(os.environ.get("XDG_CACHE_HOME") or (Path.home() / ".cache"))
        path = root / "mireviewer" / "git-askpass.sh"
        body = _ASKPASS_SH
...
def _kill_git(proc: subprocess.Popen[str]) -> None:
    from mireviewer.cleanup.kill import kill_job_tree
...
def delete_clone(path: Optional[Path], retries: int = 8) -> None:
    ...
    from mireviewer.cleanup.end import delete_clone_path
