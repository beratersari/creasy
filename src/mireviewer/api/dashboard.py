...
from mireviewer.api.report import build_report_context
from mireviewer.api.web_mimetypes import ensure_spa_mimetypes, media_type_for_path
from mireviewer.jobs.models import ERROR_STATUSES, dashboard_visible
from mireviewer.logging import get_logger, log_ok, read_job_log_lines
from mireviewer.settings import (
    SettingsError,
    save_runtime_settings,
    suggested_models,
)
from mireviewer.opencode.serve import read_serve_log, serve_log_path
from mireviewer.opencode.session import fetch_live_chat
from mireviewer.workspace.identity import mr_key
...
def api_meta(request: Request) -> dict:
    ...
    return {"version": __version__, "server_time": _now(), "app_name": PRODUCT_NAME}
...
def attach_spa(app) -> None:
    ...
    def root() -> FileResponse:
        if index.is_file():
            return _index()
        return JSONResponse({"service": "mireviewer", "docs": "/jobs"})
