...
@router.post("/mireviewer/webhook/azure")
@router.post("/creasy/webhook/azure", include_in_schema=False)
async def webhook_azure(request: Request) -> JSONResponse:
    ...
    if azure_is_reviewer_list_event(payload):
        ...
        log_ok(logger, "azure reviewers GET", project=locator[0], repo=locator[1], pr=locator[2], count=len(live))
