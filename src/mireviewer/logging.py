...
from mireviewer import log_context
...
def setup_logging(level: str = "INFO", log_dir: Path | None = None) -> logging.Logger:
    logger = logging.getLogger("mireviewer")
    fmt = _Formatter()
...
def get_logger(name: str | None = None) -> logging.Logger:
    if name:
        return logging.getLogger(f"mireviewer.{name}" if not name.startswith("mireviewer") else name)
    return logging.getLogger("mireviewer")
