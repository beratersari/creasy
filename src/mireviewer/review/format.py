...
from mireviewer.review.mention import USAGE_HEADING, USAGE_MARKER
...
def format_success(job: JobRecord) -> str:
    ...
    return f"**{PRODUCT_NAME} {__version__} — {kind}** · `{model}` · `{job.job_id}`\n\n{body}\n"
...
def format_failure(job: JobRecord) -> str:
    ...
    return f"**{PRODUCT_NAME} {__version__} — Review failed** · `{job.job_id}`\n\n```\n{err}\n```\n"


def format_cancelled(job: JobRecord) -> str:
    return f"**{PRODUCT_NAME} {__version__} — Cancelled** · `{job.job_id}` was cancelled.\n"
