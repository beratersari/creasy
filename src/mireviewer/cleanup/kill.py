...
from mireviewer.logging import get_logger, log_command, log_command_result

logger = get_logger("cleanup.kill")
...
def kill_pid(pid: Optional[int]) -> None:
    ...
    if not may_kill(resolved):
        logger.warning("refusing to kill protected pid %s (never kill creasy)", resolved)
        return
    try:
        if os.name == "nt":
...
def kill_job_tree(pids: Iterable[Optional[int]]) -> None:
    ...
    for raw in seq:
        ...
        if not may_kill(pid) or pid in guarded:
            logger.warning("refusing to kill protected pid %s (never kill creasy)", pid)
            continue
        try:
...
def query_windows_restart_manager(path: Path) -> RmHelperResult:
    ...
    cmd = [
        ...
        (
            "import json,sys;"
            "from mireviewer.cleanup.kill import _rm_query_pids;"
