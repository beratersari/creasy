"""Runtime review settings. Dashboard may change model and timeout."""
...
def remember_env_defaults(cfg: Config) -> None:
    if not (cfg.opencode_model_env or "").strip():
        cfg.opencode_model_env = (cfg.opencode_model or "").strip()
    if not cfg.opencode_timeout_env:
...
def apply_runtime_settings(cfg: Config) -> None:
    remember_env_defaults(cfg)
    data = load_overrides(cfg)
    if not data:
        return
    raw_model = data.get("opencode_model")
...
def save_runtime_settings(cfg: Config, *, model: str, timeout: int) -> None:
    remember_env_defaults(cfg)
    model = normalize_model(model)
    timeout = normalize_timeout(timeout)
    path = settings_path(cfg)
    path.parent.mkdir(parents=True, exist_ok=True)
    payload = {"opencode_model": model, "opencode_timeout": timeout}
    ...
    log_ok(logger, "settings saved", model=model, timeout=timeout)


def suggested_models(cfg: Config, extra: Iterable[str] = ()) -> list[str]:
    seen: list[str] = []
    for raw in (
        cfg.opencode_model,
